...
export const getPublicKeyTool = {
  name: "get_public_key",
  description:
    "Get the derived public key for a key. " +
...
export const exportKeyTool = {
  name: "export_key",
  description:
    "Export a key from TEE. The key material is returned encrypted with your ephemeral public key (ECIES). " +
...
export async function handleListKeys(
  ...
  args: { page?: number; size?: number; keyCode?: string; name?: string; orderDirection?: string }
) {
  const params = new URLSearchParams();
  if (args.page)           params.set("page", String(args.page));
  if (args.size)           params.set("size", String(args.size));
  if (args.keyCode)        params.set("keyCode", args.keyCode);
...
export async function handleUpdateKey(
  config: KeygenixConfig,
  args: { keyCode: string; name?: string; remark?: string }
) {
...
export async function handleGetPublicKey(
  config: KeygenixConfig,
  args: { keyCode: string; curve?: string; path?: string; deriveType?: string }
) {
  const params = new URLSearchParams();
  if (args.curve)      params.set("curve", args.curve);
  if (args.path)       params.set("path", args.path);
  if (args.deriveType) params.set("deriveType", args.deriveType);
  const qs = params.toString() ? `?${params.toString()}` : "";
...
export async function handleCreateKey(
  config: KeygenixConfig,
  args: { keyType: string; curve?: string; chains?: string[] }
) {
  const authPubKey = bytesToHex(secp256k1.getPublicKey(hexToBytes(config.authPrivKey)));

  const body: Record<string, unknown> = { keyType: args.keyType, authPubKey };

  // curve required for private/secret
  if (args.keyType === "private" || args.keyType === "secret") {
    if (!args.curve) throw new Error(`curve is required for keyType=${args.keyType}`);
  ...
  if (args.keyType === "mnemonic") {
    ...
    if (createAddresses.length > 0) body.createAddresses = createAddresses;
  }

  return apiCall(config, "POST", walletUrl(config, "/keys"), body);
}

export async function handleExportKey(
  config: KeygenixConfig,
  args: { keyCode: string }
) {
  ...
  const result = await apiCall<{ encryptedExportingKey: string }>(
    config,
    "POST",
    walletUrl(config, `/keys/${args.keyCode}/export`),
    { authSignature, timestamp, publicKey }
  );
